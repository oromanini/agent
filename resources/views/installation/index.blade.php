    ...
    <div class="container is-fluid overflow-auto">
        ...
        <form action="{{ route('installation.index') }}" method="get">
            ...
            <div id="client-filters" class="a-filterbar">
                ...
                <div style="display:flex; gap:.5rem;">
                    ...
                    <a href="{{ route('installation.index') }}" class="a-btn-ghost">Limpar</a>
                </div>
            </div>
        </form>
        ...
        <div class="a-table-wrap">
            <table class="table is-hoverable is-fullwidth">
                    ...
                    <tr>
                        <td>
                            <div class="a-name-cell">
                                <x-avatar :name="$installation->proposal->client->name" />
                                <span style="font-weight:600;">{{ $installation->proposal->client->name }}</span>
                            </div>
                        </td>
                        <td>
                            <span class="tag @if($installation->status->is_final) is-success @elseif($installation->status->name == 'Pendente') @else is-info @endif">{{ $installation->status->name }}</span>
                        </td>
                        <td>
                            <span class="tag {{ deadLineColor($installation->status, $installation->created_at->diffInDays(now())) }}">
                        ...
                        <td style="font-weight:700;">R$ {{ floatToMoney($installation->proposal->valueHistory->final_price) }}</td>
                        <td>
                            <div class="a-row-actions">
                                <a class="a-iconbtn" title="Abrir" href="{{ route('installation.show', [$installation->id]) . '#installation' }}">
                                    <ion-icon name="create-outline"></ion-icon>
                                </a>
                                <a class="a-iconbtn" title="Excluir" href="{{ route('installation.inactive', [$installation->id]) }}">
                ...
                @empty
                    <tr>
                        <td colspan="6">Não há instalações disponíveis</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-4">{{ $installations->appends(request()->all())->links() }}</div>
    </div>
@endsection
